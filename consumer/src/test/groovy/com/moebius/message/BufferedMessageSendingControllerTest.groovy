...
class BufferedMessageSendingControllerTest extends Specification {
    ...
    def "Test not sending when all messages on buffer are saved after deadline"() {
        given:
        def messagesInBuffer = [
    ...
    def "Test sending messages when all messages on buffer are saved before deadline and has dedup strategy of LEAVE_LAST_ARRIVAL"() {
        given:
        def messagesInBuffer = [
                createBufferedMessages(DedupStrategy.LEAVE_LAST_ARRIVAL, 5, -10),
                createBufferedMessages(DedupStrategy.LEAVE_LAST_ARRIVAL, 10, -5),
        ...
        5 * messageSender.sendMessage({
            it.dedupStrategy == DedupStrategy.LEAVE_LAST_ARRIVAL
        }) >> Mono.just(true)
        5 * messageSendingBuffer.dropKey(_ as String) >> Mono.just(true)
    ...
    BufferedMessages createBufferedMessages(DedupStrategy dedupStrategy, int msgCount, int minutesToAddDeadLine) {
        ...
        def msgRequests = IntStream.range(0, msgCount).mapToObj({
            new MessageSendRequest(dedupStrategy, "title-${it}", Mock(MessageBody), mockRecipient)
        }).collect(Collectors.toList())

        return new BufferedMessages(dedupStrategy, firstReceivedTime, msgRequests, messageKey)
    }
}
